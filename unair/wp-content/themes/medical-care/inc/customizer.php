...
function medical_care_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting( 'medical_care_reset_colors', array(
	    'default'           => '',
	    'sanitize_callback' => 'sanitize_text_field',
	    'transport'         => 'postMessage', 
	) );

	$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'medical_care_reset_colors', array(
	    'label'       => esc_html__( 'Reset Colors', 'medical-care' ),
	    'section'     => 'colors',
	    'settings'    => 'medical_care_reset_colors',
	    'type'        => 'button',
	    'input_attrs' => array(
	...
	$wp_customize->add_section('medical_care_blog_meta',array(
        'title' => __('Blog Meta', 'medical-care'), 
        'panel' => 'medical_care_post_panel',       
    ) );

    $wp_customize->add_setting( 'medical_care_section_blog_meta_heading', array(
		'default'           => '',
		'transport'         => 'refresh',
		'sanitize_callback' => 'sanitize_text_field',
	) );
	$wp_customize->add_control( new Medical_Care_Customizer_Customcontrol_Section_Heading( $wp_customize, 'medical_care_section_blog_meta_heading', array(
		'label'       => esc_html__( 'Blog Meta Settings', 'medical-care' ),
		'section'     => 'medical_care_blog_meta',
		'settings'    => 'medical_care_section_blog_meta_heading',
	) ) );
	...
	$wp_customize->add_setting('medical_care_date_type',array(
        'default' => 'published',
        'sanitize_callback' => 'medical_care_sanitize_choices'
	));
	$wp_customize->add_control('medical_care_date_type',array(
		'type' => 'radio',
		'label'     => __('Date Format', 'medical-care'),
		'section' => 'medical_care_blog_meta',
		'type' => 'select',
		'choices' => array(
			'published' => __('published date','medical-care'),
            'modified' => __('modified date','medical-care'),
		),
	) );
	...
	$wp_customize->add_control('medical_care_grid_column',array(
		'type' => 'radio',
		'label'     => __('Grid Post Per Row', 'medical-care'),
		'section' => 'medical_care_archieve_post_layot',
		'type' => 'select',
		'choices' => array(
	...
	if ( class_exists( 'WooCommerce' ) ) { 
		...
		$wp_customize->add_control( new Medical_Care_Customizer_Customcontrol_Section_Heading( $wp_customize, 'medical_care_section_shoppage_heading', array(
			'label'       => esc_html__( 'Sidebar Settings', 'medical-care' ),
			'section'     => 'medical_care_woocoomerce_section',
    ...
    public function render_content(){?>
        <label style="overflow: hidden; zoom: 1;">
	        ...
	        <div class="col-md-3 col-sm-6">
	            <h3 style="margin-top:10px; margin-left: 20px; font-size:12px; text-decoration:underline; color:#333;"><?php esc_html_e('MEDICAL PREMIUM - Features', 'medical-care'); ?></h3>

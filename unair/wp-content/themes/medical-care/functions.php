...
// Date formatting
function medical_care_display_shop_date() {
    // Get the date type option
    $medical_care_date_type = get_theme_mod( 'medical_care_date_type', 'published' );

    // Determine the date to display based on the type
    if ( $medical_care_date_type === 'modified' && get_the_modified_time( 'U' ) !== get_the_time( 'U' ) ) {
        $medical_care_date_to_display = get_the_modified_date( get_option( 'date_format' ) );
    } else {
        $medical_care_date_to_display = get_the_date( get_option( 'date_format' ) );
    }

    // Output the date HTML

    echo esc_html( $medical_care_date_to_display );
}

...
function expert_healthcare_breadcrumbs() {
	...
	$seprator	= get_theme_mod('expert_healthcare_breadcrumb_seprator','/');
	global $post;
	$homeLink = home_url();
...
function expert_healthcare_default_styles() {
    // Begin Style
    $css = '<style>';

    $logo_width = get_theme_mod('expert_healthcare_logo_width');
    $css .= '
...
add_action('wp_head', 'expert_healthcare_default_styles', 999); // Use a priority to ensure it loads after other styles
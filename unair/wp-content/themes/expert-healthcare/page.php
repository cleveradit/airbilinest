...
<section class="blog-area inarea-blog-2-column-area three">
	<div class="container">
		<div class="row">
			<?php 
                $expert_healthcare_paged_sidebar_setting = get_theme_mod('expert_healthcare_paged_sidebar_setting','1');
                $expert_healthcare_sidebar_position = get_theme_mod('expert_healthcare_sidebar_position', 'right');
                $expert_healthcare_content_class = ($expert_healthcare_paged_sidebar_setting == '') ? 'col-lg-12' : 'col-lg-8';
	            ...
	            <?php 
	                if ( class_exists( 'woocommerce' ) ) {
	                    if( is_account_page() || is_cart() || is_checkout() ) {
	                        echo '<div class="' . esc_attr($expert_healthcare_content_class . ' ' . $expert_healthcare_content_order_class) . '">';
	                    }
	                    else{
	                        echo '<div class="' . esc_attr($expert_healthcare_content_class . ' ' . $expert_healthcare_content_order_class) . '">';
	                    }
			...
			<?php if( $expert_healthcare_paged_sidebar_setting != '') { ?> 
                <?php get_sidebar(); ?>
            <?php } ?>
		</div>
	</div>
